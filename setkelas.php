<?php
     ...
     $sql = mysqli_query($conn,"select * from tblsiswa where id NOT IN (select idsiswa from tbsetkelas where tahun = '2023/2024')");
     $kelas = mysqli_query($conn,"select * from tbkelas order by nama_kelas asc");
     $setkelas = mysqli_query($conn,"select a.nama_siswa, b.nama_kelas,c.tahun from tblsiswa a, tbkelas b, tbsetkelas c where a.id=c.idsiswa AND b.idkelas=c.idkelas order by b.nama_kelas, a.nama_siswa asc");
?>
